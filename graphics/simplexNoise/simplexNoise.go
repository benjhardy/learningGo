...
// networking
// Threadings
// would be cool to add colors into the make noise inputs to change colors on the fly
package main
...
func colorLerp(c1, c2 color, pct float32) color {
	c := color{lerp(c1.r, c2.r, pct), lerp(c1.g, c2.g, pct), lerp(c1.b, c2.b, pct)}
	return c
}
...
func makeNoise(pixels []byte, frequency, gain, lac float32, octaves int) {

	noise := make([]float32, winWidth*winHeight)
	fmt.Println("freq: ", frequency, "lacunarity: ", lac, "gain: ", gain, "octaves: ", octaves)
	i := 0
	min := float32(9999.0)
	max := float32(-9999.0)
...
func main() {
	...
	makeNoise(pixels, frequency, gain, lac, octaves)
